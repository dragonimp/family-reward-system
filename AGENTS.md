...
- 每次任务完成并通过必要验证后，必须在当前任务中立即创建聚焦的 Git 提交；不得等待其他任务或长期留在工作区。
- 部署前必须盘点本仓库的所有本地分支和 Git 工作树，将所有已完成且已提交的成果合并到本次发布目标分支；完成成果未合并时禁止部署。
- 仍在进行、已废弃或实验性的分支/工作树可不合并，但必须在部署记录中明确列出状态和排除原因，不得以此漏合已完成成果。
- 合并完成后必须确认发布目标分支工作树干净、已完成分支相对目标分支不再存在未集成提交，然后立即使用项目正式脚本部署并完成线上验证；不得无故拖延或改用手工上传。
