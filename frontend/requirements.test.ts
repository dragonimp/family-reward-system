...
test('REQ-067 adds unrated parent warm moments and traceable dual growth reports', async () => {
  ...
  assert.match(dashboard, /今日双向成长/);
  assert.match(dashboard, /孩子成长/);
  assert.match(dashboard, /爸妈成长/);
  assert.match(api, /写给孩子本人，用第二人称、儿童能懂的短句，以具体鼓励为主/);
  assert.match(api, /generated_by='ai-v2'/);
  assert.match(api, /DateOnly\.FromDateTime\(DateTime\.UtcNow\.AddHours\(8\)\)/);
  const watchReport = api.slice(api.indexOf("document.getElementById('watch-report').innerHTML"), api.indexOf("const child = (score.children"));
  assert.match(watchReport, /report\.praise/);
  assert.match(watchReport, /report\.nextStep/);
  assert.doesNotMatch(watchReport, /report\.changeSummary/);
  assert.match(growth, /做得好的点|report\.praise/);
  assert.match(growth, /孩子成长/);
  assert.match(growth, /爸妈成长/);
