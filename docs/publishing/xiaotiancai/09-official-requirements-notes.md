...
- 官网当前直接提供 `应用测试check表及标准V1.7.xlsx`、`XX应用-测试用例范例.xlsx`、`应用性能测试报告模板V1.3.xlsx` 和 `应用测试流程指导.docx`；正式结果必须按这些原件填写，项目自制 PDF 只作补充。
- 官网当前“审核规范”要求冷启动首帧完成小于 1 秒、页面可交互小于 1.5 秒、页面或功能切换不超过 500 毫秒、前台 CPU 小于 70%、普通应用后台息屏 CPU 为 0%、内存均值不超过 60 MB、峰值不超过 160 MB、卡顿率不超过 0.1%。
- 稳定性准出要求 Monkey 和自动化遍历各累计 18 小时、最小连续 8 小时、覆盖三层及以上页面，日志中 0 Crash/ANR/内存泄漏。
- V1.7 check 表还要求连续使用 30 分钟耗电不超过 20%；纯浏览类 4G 连续使用 30 分钟流量不超过 5 MB；正式测试需回填功耗/流量、待机、缺陷和内容审核工作表。
- 每次提审 `versionCode` 必须递增。
- 已上线应用的 `packageName` 不能变更。
- 官方建议同一 APK 兼容多机型，可在代码中根据 `android.os.Build.MODEL` 做机型适配。

官网原件仍包含不同年代的示例机型和历史文字：流程指导举例 Z2/Z3/Z5，V1.3 性能模板对标 Z12，V1.7 check 表包含早期平台阈值。正式测试机型由小天才确认，不把本项目推测的 Z8/Z9 写成官网指定机型。当前页面的邮箱、模板版本和审核规范优先于旧模板中的历史字段。
