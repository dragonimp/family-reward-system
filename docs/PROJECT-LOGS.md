...
## [2026-09-06] family-reward-REQ-066 方形表盘测试用例设计
- Orbit：`gpt_cf29feecba5545d8881735db0b1a0ef8`；需求：`family-reward-REQ-066`（`6eba3ec2-913d-4a3a-b30b-8cf77d1cc805`）；测试设计任务：`family-reward-TASK-252`（`e186ec56-4ed2-4431-bd75-d8385bbbce0d`）；功能点：`245414ae-7163-46af-82bb-1bb73cf9a63b`。
- 新增方形表盘静态测试用例与追踪矩阵，覆盖统一外观、基础流程、尺寸、小天才/华为/小米、可访问性和业务无副作用。
- TASK-252 可凭文档、静态测试和提交关闭；REQ-066 的三类目标设备、完整业务回归及受控部署后线上复测证据未齐，不按完整验收关闭。

## [2026-09-05] family-reward-REQ-066 方形手表表盘实现
- 将 `/watch` 的主表盘、内描边和所有主题裁剪边界统一改为直角方形，积分环继续保留圆形语义。
- 表盘尺寸新增 `100vmin` 旧 WebView 回退和 `aspect-ratio` 现代约束；菜单按钮移入方形右上安全区，补充 260px 以下受限视口规则。
- 新增 REQ-066 静态回归，覆盖方形外观、兼容回退、窄屏规则和菜单非负偏移；未改变表盘 API、VIP 授权或积分奖励业务。

## [2026-09-02] family-reward-REQ-067 亲子双向成长调整需求分析
- Orbit：`gpt_c4eacf8b958849f7afee751991f252ee`；需求：`family-reward-REQ-067`（`f5eb49bd-43cf-42c5-94b0-cc32567286b7`）；分析任务：`family-reward-TASK-247`。
- 新增 `docs/REQ-067-ANALYSIS.md`，将 V0.8 后的升级定义为独立“成长事实—AI 总结—日/周/月报告”链路；保留积分交易模型，暖心内容不产生积分、不进入任何排行。
- 明确手表家长选择和文字/STT 确认流程、家庭与绑定权限、独立数据模型、版本化/幂等生成、原始记录追溯、删除传播、未成年人隐私和禁用评价/诊断表达护栏。
- Web 统计默认改为同一主体的历史周期比较，好友排行新旧家庭均默认关闭；即使主动开启，暖心记录、总结、报告及其数量也永久排除在排行数据源之外。
- `TASK-247` 可凭分析文档和对应提交关闭；REQ-067 尚未实施，仍需完成开发、AI 安全评测、隐私/权限/积分回归、目标手表验证和受控部署后的线上复测，当前不可关闭需求。

## [2026-09-02] V0.8 产品基线冻结
- 将当前家庭积分产品保存为 `V0.8`，新增根目录 `VERSION` 和 `docs/releases/V0.8.md`，并在项目首页与状态文档中登记当前产品版本。
- 本版本冻结家长 Web、儿童手表、家庭圈子、孩子账户、规则与交易、统计、设备、好友、智能体和 MCP 等已经提交到 `main` 的能力。
- 明确 V0.8 是亲子双向成长升级前的基线；孩子记录家长闪光时刻、双向成长账本、成长日历和周报/月报不属于本版本已实现范围。
- 产品版本与后端服务版本、手表 Android 安装包版本保持独立；本次不修改后端 `3.1.0` 或手表 `1.0.0 (100)` 的组件版本口径。
- 工作区原有 `bin/`、`obj/`、`frontend/dist/` 构建产物变更未纳入版本提交，正式产物应从 `v0.8` 标签对应源码重新构建。
